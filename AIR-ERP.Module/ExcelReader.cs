...
namespace utilities
{
	...
	public class ExcelReader : IDisposable
	{
		...
		{
			return
				String.Format(@"Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0};Extended Properties={1}Excel 8.0;{2}{1}", ExcelFilename, Convert.ToChar(34), ExcelConnectionOptions()); 
		}
		#endregion
		...
		public void Open()
		{
			try
			{
				...
				if (System.IO.File.Exists(ExcelFilename)==false)
				{
					throw new Exception(String.Format("Excel file {0}could not be found.", ExcelFilename));
				}
		...
		private bool SetSheetQuerySelect()
		{
			try
			{
				...
				_oleCmdSelect =new OleDbCommand(
					String.Format(@"SELECT * FROM [{0}${1}]", SheetName, _strSheetRange), _oleConn);   
		...
		private OleDbDataAdapter SetSheetQueryAdapter(DataTable dt)
		{
			...
			try
			{
				...
				{
					strWhere = AddWithAnd(strWhere,String.Format("{0}=?", dt.Columns[iPK].ColumnName)); 
				}
				...
				for (int iCol=0;iCol<dt.Columns.Count;iCol++)
				{
					...
					strUpdate= String.Format("{0}=?", AddWithComma(strUpdate,dt.Columns[iCol].ColumnName));
				}

				string strTable = String.Format("[{0}${1}]", SheetName, SheetRange);  
				strInsert = String.Format("INSERT INTO {0}({1}) Values ({2})", strTable, strInsert, strInsertPar);
				strUpdate = String.Format("Update {0} Set {1}{2}", strTable, strUpdate, strWhere);
		...
		private bool SetSheetQuerySingelValUpdate(string strVal)
		{
			try
			{
				...
				_oleCmdUpdate =new OleDbCommand(
					String.Format(@" Update [{0}${1}] set F1={2}", SheetName, _strSheetRange, strVal), _oleConn);   
				return true;
			}			
			catch (Exception ex)
			{
	...
	public DataTable GetTable(string strTableName)
		
		{
			try
			{
				...
				//Fill table
                OleDbDataAdapter oleAdapter = new OleDbDataAdapter { SelectCommand = _oleCmdSelect };   
		...
		{
			_strSheetRange = String.Format("{0}:{0}", strCell);
		}
